namespace Inmobiliaria.Models
{
  ...
  public class RepositorioUsuario : RepositorioBase, IRepositorioUsuario
  {
    ...
    public Usuario ObtenerPorEmail(string email)
    {
      ...
      using (var conn = new MySqlConnection(connectionString))
      {
        var sql = @"SELECT id_usuario, nombre, apellido, genero, email, contrasena, rol, url, estado, created_at, updated_at
                    FROM usuario 
                    WHERE email = @Email;";
        using (var cmd = new MySqlCommand(sql, conn))
        {
    ...
    public int Modificar(Usuario usuario)
    {
      ...
      using (var conn = new MySqlConnection(connectionString))
      {
        var sqlBuilder = new System.Text.StringBuilder(@"UPDATE usuario
            SET 
                nombre = @nombre,
                apellido = @apellido,
                email = @email,
        ...
        using (var cmd = new MySqlCommand(sqlBuilder.ToString(), conn))
        {
          ...
          cmd.Parameters.AddWithValue("@url", !string.IsNullOrEmpty(usuario.AvatarURL) ? (object)usuario.AvatarURL : DBNull.Value);
          cmd.Parameters.AddWithValue("@estado", usuario.Estado);
    ...
    private Usuario MapUsuario(MySqlDataReader reader)
    {
      return new Usuario
      {
        ...
        AvatarURL = reader.IsDBNull(reader.GetOrdinal("url")) ? "" : reader.GetString("url"),
    ...
    public int CantidadFiltro(string? idUsuario, string? nombre, string? apellido, string? email, string? rol, string? estado)
    {
      int total = 0;
      using (var conn = new MySqlConnection(connectionString))
      {
    ...
    public List<Usuario> Filtrar(string? idUsuario, string? nombre, string? apellido, string? email, string? rol, string? estado, int? limit, int? offset)
    {
