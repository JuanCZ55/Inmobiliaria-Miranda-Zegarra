...
public class Inquilino
{
    ...
    [RegularExpression(
        @"^\d{7,8}$",
        ErrorMessage = "El DNI debe tener solo numeros y entre 7 y 8 digitos"
    )]
    ...
    [RegularExpression(
        @"^\+?\d{8,15}$",
        ErrorMessage = "El telefono debe tener solo numeros y puede empezar con +, entre 8 y 15 digitos"
    )]
    ...
    [RegularExpression(
        @"^(?=.*[A-Za-z])[\w\s,./-]+$",
        ErrorMessage = "La direccion puede ser como: Av. Libertador 2500 / Piso 3 - Calle Falsa S/N - Ruta 3 km 45"
    )]
