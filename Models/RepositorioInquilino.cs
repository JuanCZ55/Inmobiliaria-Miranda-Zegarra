namespace Inmobiliaria.Models
{
    ...
    public class RepositorioInquilino : RepositorioBase, IRepositorioInquilino
    {
        ...
        public int Crear(Inquilino inquilino)
        {
            ...
            {
                var sql =
                    @"INSERT INTO inquilino (dni, nombre, apellido, telefono, email, direccion, estado) VALUES (@dni, @nombre, @apellido, @telefono, @email, @direccion, @estado); SELECT LAST_INSERT_ID();";
        ...
        public int Modificar(Inquilino inquilino)
        {
            ...
            {
                var sql =
                    @"UPDATE inquilino SET dni=@dni, nombre=@nombre, apellido=@apellido, telefono=@telefono, email=@email, direccion=@direccion, estado=@estado, updated_at=NOW() WHERE id_inquilino=@id";
                using (var cmd = new MySqlCommand(sql, conn))
                {
                    cmd.Parameters.AddWithValue("@dni", inquilino.Dni);
                    cmd.Parameters.AddWithValue("@nombre", inquilino.Nombre);
        ...
        public Inquilino ObtenerPorID(int id)
        {
            ...
            {
                var sql =
                    @"SELECT id_inquilino, dni, nombre, apellido, telefono, email, direccion, estado, created_at, updated_at FROM inquilino WHERE id_inquilino = @id";
                using (var cmd = new MySqlCommand(sql, conn))
                {
                    cmd.Parameters.AddWithValue("@id", id);
                    conn.Open();
                    using (var reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            i.IdInquilino = reader.GetInt32("id_inquilino")!;
                            i.Dni = reader.GetString("dni")!;
                            i.Nombre = reader.GetString("nombre")!;
                            i.Apellido = reader.GetString("apellido")!;
                            i.Telefono = reader.GetString("telefono")!;
                            i.Email = reader.GetString("email")!;
        ...
        public Inquilino ObtenerPorDni(string dni)
        {
            ...
            {
                var sql =
                    @"SELECT id_inquilino, dni, nombre, apellido, telefono, email, direccion, estado, created_at, updated_at FROM inquilino WHERE dni = @dni";
                using (var cmd = new MySqlCommand(sql, conn))
                {
                    cmd.Parameters.AddWithValue("@dni", dni);
                    conn.Open();
        ...
        public List<Inquilino> BuscarPorNombre(string nombre)
        {
            ...
            {
                var sql =
                    @"SELECT id_inquilino, dni, nombre, apellido, telefono, email, direccion, estado, created_at, updated_at FROM inquilino WHERE nombre LIKE @nombre";
                using (var cmd = new MySqlCommand(sql, conn))
                {
        ...
        public List<Inquilino> filtrarDNI(string dni)
        {
            ...
            {
                var sql =
                    @"SELECT id_inquilino, dni, nombre, apellido, telefono, email, direccion, estado, created_at, updated_at FROM inquilino WHERE dni LIKE CONCAT(@dni, '%')";
                using (var cmd = new MySqlCommand(sql, conn))
                {
                    cmd.Parameters.AddWithValue("@dni", dni);
                    conn.Open();
        ...
        public List<Inquilino> ObtenerTodos()
        {
            ...
            {
                var sql =
                    @"SELECT id_inquilino, dni, nombre, apellido, telefono, email, direccion, estado, created_at, updated_at FROM inquilino ORDER BY id_inquilino DESC;";
                using (var cmd = new MySqlCommand(sql, conn))
                {
                    conn.Open();
                    using (var reader = cmd.ExecuteReader())
