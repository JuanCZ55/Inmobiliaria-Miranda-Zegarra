...
public class UsuarioController : Controller
{
    ...
    public async Task<IActionResult> Registrar(Usuario user, string ConfirmPassword)
    {
        ...
        {
            TempData["Error"] =
                "Ocurrió un error inesperado al registrar el usuario: " + ex.Message;
    ...
    )
    {
        ...
        try
        {
            ...
            if (
                int.TryParse(idUsuarioLogueadoClaim, out int idUsuarioLogueado)
                && usuarioOriginal.IdUsuario == idUsuarioLogueado
            )
            {
            ...
            if (!string.IsNullOrEmpty(NewPassword))
            {
                ...
                usuarioOriginal.Password = passwordHasher.HashPassword(
                    usuarioOriginal,
                    NewPassword
                );
            }
            ...
            if (user.AvatarFile != null && user.AvatarFile.Length > 0)
            {
                ...
                {
                    if (
                        !usuarioOriginal.AvatarURL.Contains("AvatarMasculino.png")
                        && !usuarioOriginal.AvatarURL.Contains("AvatarFemenino.png")
                    )
                    {
            ...
            if (usuarioOriginal.IdUsuario == idUsuarioLogueado)
            {
                ...
                var claims = new List<Claim>
                {
                    ...
                    new Claim(
                        ClaimTypes.Role,
                        usuarioOriginal.Rol == 1 ? "Administrador" : "Empleado"
                    ),
                    new Claim("IdUsuario", usuarioOriginal.IdUsuario.ToString()),
                    new Claim(
                        "AvatarURL",
                        !string.IsNullOrEmpty(usuarioOriginal.AvatarURL)
                            ? usuarioOriginal.AvatarURL
                            : GetDefaultAvatar(usuarioOriginal)
                    ),
    ...
    public IActionResult Ver(int IdUsuario)
    {
        try
        {
            ...
            usuario.AvatarURL = !string.IsNullOrEmpty(usuario.AvatarURL)
                ? usuario.AvatarURL
                : GetDefaultAvatar(usuario);
    ...
    public IActionResult Perfil()
    {
        ...
        var data = new
        {
            ...
            AvatarURL = !string.IsNullOrEmpty(usuario.AvatarURL)
                ? usuario.AvatarURL
                : GetDefaultAvatar(usuario),
        };
    ...
    public async Task<IActionResult> EditarPerfil([FromForm] Usuario usuario)
    {
        ...
        if (
            idUsuarioClaim == null
            || !int.TryParse(idUsuarioClaim.Value, out int idUsuarioLogueado)
        )
        {
            return Unauthorized(
                new { message = "Sesión inválida. No se pudo identificar al usuario." }
            );
        }
        ...
        try
        {
                ...
                var claims = new List<Claim>
                {
                    ...
                    new Claim(
                        ...
                        !string.IsNullOrEmpty(usuarioOriginal.AvatarURL)
                            ? usuarioOriginal.AvatarURL
                            : GetDefaultAvatar(usuarioOriginal)
                    ), // Actualizar el claim del avatar
                };
                var claimsIdentity = new ClaimsIdentity(
                    claims,
                    CookieAuthenticationDefaults.AuthenticationScheme
            ...
            return Ok(
                new
                {
                    ...
                    newAvatarUrl = !string.IsNullOrEmpty(usuarioOriginal.AvatarURL)
                        ? usuarioOriginal.AvatarURL
                        : GetDefaultAvatar(usuarioOriginal),
                }
            );
        ...
        catch (Exception)
        {
            return StatusCode(
                500,
                new { message = "Ocurrió un error inesperado al actualizar el perfil." }
            );
        }
    }
    ...
    )
    {
        ...
        catch (Exception)
        {
            return StatusCode(
                500,
                new { message = "Ocurrió un error inesperado al cambiar la contraseña." }
            );
        }
    }
    ...
    public IActionResult Eliminar(int id)
    {
        try
        {
            ...
            if (
                !string.IsNullOrEmpty(usuarioAEliminar.AvatarURL)
                && !usuarioAEliminar.AvatarURL.Contains("AvatarMasculino.png")
                && !usuarioAEliminar.AvatarURL.Contains("AvatarFemenino.png")
            )
            {
